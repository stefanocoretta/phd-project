...
splines.raw <- list.files(path = "./italian-polish/results/cached",
                   pattern = "*-aaa.csv",
                   full.names = TRUE) %>%
    map_df(~read.delim(., col.names = columns, na.strings = "*"))

splines.closure.raw <- list.files(path = "./italian-polish/results/cached",
                   pattern = "*-closure.csv",
                   full.names = TRUE) %>%
    map_df(~read.delim(., col.names = columns, na.strings = "*"))

splines.phantom.raw <- list.files(path = "./italian-polish/results/cached",
                   pattern = "*-phantom.csv",
                   full.names = TRUE) %>%
...
filter(splines.it, label == "max_TT") %>%
    ggplot(aes(fan, Y, colour = c2phonation, group = rec.date)) +
...
## GAMMs at maximum constriction pl03
...
## GAMMs at maximum constriction pl02

```{r max-pl-pl02}
max.pl.pl02 <- splines.pl %>%
    filter(label %in% c("max_TT", "max_TD"),
           speaker == "pl02",
           vowel != "u",
           X < 40,
...
summary(max.pl.gamm.pl02)
```

### AR1 model

```{r max-pl-gam-ar-pl02}
r1 <- start_value_rho(max.pl.gamm.pl02)

max.pl.gam.pl02.AR <- bam(
    Y ~
        c2phonation.ord +
        c2place.ord +
        vowel.ord +
    ...
    method = "fREML",
    rho = r1,
    AR.start = max.pl.pl02$start.event
)

summary(max.pl.gam.pl02.AR)

max.pl.gam.pl02.AR.null <- bam(
    Y ~
#        c2phonation.ord +
        c2place.ord +
        vowel.ord +
...
compareML(max.pl.gam.pl02.AR.null, max.pl.gam.pl02.AR)
```

```{r max-pl-gam-ar-acf-pl02}
acf_plot(resid(max.pl.gam.pl02.AR), split_by=list(max.pl.pl02$rec.date))
acf_resid(max.pl.gam.pl02.AR, split_pred = "AR.start")
```

```{r max-pl-gam-ar-plot-pl02}
plot_smooth(max.pl.gam.pl02.AR, view = "X",
            plot_all= "c2phonation.ord", rug = FALSE)
plot_diff(max.pl.gam.pl02.AR, view = "X",
          comp = list(c2phonation.ord = c("voiceless", "voiced")))
```
...
closure.pl <- splines.closure.pl %>%
    filter(speaker == "pl03", vowel != "u", X > -15) %>%
    na.omit() %>%
    arrange(rec.date, fan) %>%
    start_event(column = "fan", event = c("rec.date"))
```
...
# Plot smooth and difference

```{r max-it-gam-combo}
diff.df <- plot_diff(max.it.gam.AR, view = "X",
          comp = list(c2phonation.ord = c("voiceless", "voiced")),
          plot = FALSE)

smooth.df <- get_predictions(max.it.gam.AR, cond = list(c2phonation.ord = c("voiceless", "voiced"), X = seq(min(diff.df$X), max(diff.df$X), length = 200)))

sig.diff <- find_difference(diff.df$est, diff.df$CI, diff.df$X)

smooth.plot <- smooth.df %>%
    ggplot(aes(X, fit)) +
    annotate("rect", xmin=sig.diff$start, xmax=sig.diff$end, ymin=-Inf, ymax=Inf, alpha=0.2, fill="red") +
    geom_ribbon(aes(ymin = fit - CI,
                ymax = fit + CI,
                fill = c2phonation.ord
                ),
            alpha = 0.2,
            colour = "NA"
            ) +
    geom_line(aes(colour = c2phonation.ord)) +
    theme(axis.title.x=element_blank(), legend.position = "top") +
    xlim(min(diff.df$X), max(diff.df$X))

diff.plot <- diff.df %>%
    ggplot(aes(X, est)) +
    geom_hline(yintercept = 0, size = 0.3) +
    annotate("rect", xmin=sig.diff$start, xmax=sig.diff$end, ymin=-Inf, ymax=Inf, alpha=0.2, fill="red") +
    geom_ribbon(aes(ymin = est - CI,
                ymax = est + CI
                ),
            alpha = 0.2
            ) +
...
```{r max-it-gam-combo-plot}
plot_grid(smooth.plot, diff.plot, align = "v", nrow = 2, rel_heights = c(2/3, 1/3))
```

```{r closure-it-gam-combo}
diff.df <- plot_diff(closure.it.gam.AR, view = "X",
          comp = list(c2phonation.ord = c("voiceless", "voiced")),
          plot = FALSE)

smooth.df <- get_predictions(closure.it.gam.AR, cond = list(c2phonation.ord = c("voiceless", "voiced"), X = seq(min(diff.df$X), max(diff.df$X), length = 200)))
...
```{r plot-smdiff}
plot_smdiff <- function(model, view, comparison) {
...
```{r plot-smdiff-max-it}
plot_smdiff(max.it.gam.AR, "X", list(c2phonation.ord = c("voiceless", "voiced")))
```

# Start event tibble functions

```{r}
event <- as.character(pull(max.it, rec.date))
previous <- ""
event.start <- NULL
