...
```{r vowdur-dens}
vowels %>%
  ggplot(aes(vowel.duration, fill = c2_phonation, colour = c2_phonation)) +
...
```{r vowdur-box}
vowels %>%
  ggplot(aes(c2_phonation, vowel.duration)) +
  geom_boxplot() +
  facet_grid(~ language)
```
...
vowels %>%
  ggplot(aes(c2_phonation, vowel.duration, fill = vowel)) +
  geom_boxplot() +
  facet_wrap(~ speaker, ncol = 4, scales = "free")
```
...
```{r it03}
it03 <- filter(splines, speaker == "it03", label %in% c("max_TT", "max_TD"))
```
...
```{r it02-clos}
it02_clos <- filter(splines_closure, speaker == "it02")
```
...
Non significant better fit with `c2_phonation`.
