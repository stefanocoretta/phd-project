...
Mean dEGG cannot accout for concomitant symmetric increase and decrease of minimum and maximum: the mean stays the same.
...
quotient %>%
  group_by(speaker) %>%
  mutate(contact_quotient_z = scale(contact_quotient)) %>%
  ggplot(aes(contact_quotient_z, colour = phonation)) +
    geom_density() +
  geom_rug() +
    facet_wrap(~ speaker)
```
...
```{r cq-density}
ggplot(quotient_mean, aes(mean_quotient, colour = phonation)) +
    geom_density() +
    facet_wrap(~ speaker)
```
...
```{r speaker-tracegram, eval=FALSE}
