...
kines_mean <- kines %>%
  ...
  summarise(trp_mean = mean(trp, na.rm = TRUE), trp_z_mean = mean(trp_z, na.rm = TRUE)) %>%
  ungroup()

trp_difference <- kines_mean %>%
  group_by(speaker) %>%
  mutate(difference = trp_mean - lag(trp_mean)) %>%
  na.omit()
```
...
```{r trp-difference}
trp_difference %>%
  ggplot(aes(reorder(speaker, difference), difference, fill = language)) +
