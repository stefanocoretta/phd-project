...
if (!file.exists("./voicing-effect/data/cache/tra-gam-2.Rdata")) {
  ...
  save(tra_gam, tra_gam_ar, tra_gam_ar_0, file = "./voicing-effect/data/cache/tra-gam-2.Rdata")
...
```{r tra-lm-plot}
plot_model(tra_lm, type = "pred", terms = c("trp"))
```
...
```{r tra-lm-2-plot}
plot_model(tra_lm_2, show.values = TRUE, show.p = FALSE, order.terms = 1:5)
```
...
```{r tra-lm-3-est}
plot_model(tra_lm_3, order.terms = 1:5)
```

```{r tra-lm-3-est-2}
plot_model(tra_lm_3, terms = "v1_duration")
```

```{r tra-lm-3-plot}
plot_model(tra_lm_3, type = "pred", terms = c("v1_duration"))
```
