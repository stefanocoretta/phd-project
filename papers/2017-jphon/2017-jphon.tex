...
\Cref{t:dur-measures}. The onset of the target consonant burst (C2
...
results in \Cref{s:splines} thus refer only to /a/ and /o/.
...
the effect (\(\beta\) = 6 ms; sd = 2; \(\chi\^2\) = 8.28; p \textless{}
...
found in previous research on English \citep{ahn2016}. \Cref{f:tra} (top
...
advanced tongue root in Italian, but not in Polish. The results indicate
that tongue root advancement in voiced stops indeed correlates with the
presence of the VE, although the details of such correlation differ from
what was expected. This section will discuss \ldots{} and the
implications of the findings reported in this paper with regard to the
source of the voicing effect.
...
as a non-voicing-effect language, which was not corroborated by our
data. Crucially, the Polish speaker with the strongest effect of voicing
(PL05, cf. \Cref{s:vow-duration}) is also the only Polish speaker who
produced voiced consonants with an advanced tongue root, both at C2
closure onset and at the time of maximum tongue displacement.
...
principle. All things considered, the following implicational statement
can be inferred: if a speaker realises voiced consonants with
concomitant advancement of the tongue root, other things being equal,
then that same same speaker will also show a considerable durational
difference in vowels, with longer vowels preceding voiced consonants.
...
to revisit the question of which is the source of the voicing effect. As
pointed out in \Cref{s:intro}, the ideal candidate should be related to
...
the results of this study show that speakers can adopt or not tongue
root advancement, but if they do, they do so in such a way that the root
in voiced stops is already advanced at the onset of the consonant
closure. This observation can only be interpreted as an indication that
the advancement of the root is initiated before the closure onset. What
this implies is that the advancement gesture is implemented even during
the articulation of the vowel.

Adapting the reasoning of the proposal by Halle and Stevens (1967, see
\Cref{s:intro}), a possible cause for the longer duration of vowels
before voiced consonants could therefore be the additional time required
to achieve tongue root advancement in the context of voiced stops. Such
gestural timing account is intended as diachronic source of the voicing
effect as a property of languages, rather than as a mechanic condition
by which the voicing effect should be observed synchronically. This is
especially true if one considers that one speaker of Italian did not
produced voiced consonants with tongue root advancement, while still
having a relatively strong voicing effect.
