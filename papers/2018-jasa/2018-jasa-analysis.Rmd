...
```{r means}
group_by(durations_filtered, language, c2_phonation) %>%
  summarise(
    vowel = mean(vowel_duration),
    vowel_sd = sd(vowel_duration),
    closure = mean(closure_duration),
    closure_sd = sd(closure_duration),
    word = mean(word_duration),
    word_sd = sd(word_duration)
  )
```

```{r means-rr}
group_by(rel_filtered, language, c2_phonation) %>%
  summarise(
    rr = mean(rel_rel),
    rr_sd = sd(rel_rel)
  )
...
```{r closure-lm-plot}
plot(allEffects(closure_lm_full)[[3]])
```
...
```{r vow-clo-lm-plot}
plot(allEffects(vow_clo_lm_full))
```

```{r vow-clo-plot}
as_tibble(effect("closure_duration:vowel", vow_clo_lm_full)) %>%
...
# Mean vowel and closure durations

```{r}
mean_duration <- group_by(durations_filtered, language, c2_phonation) %>% summarise(v.mean = mean(vowel_duration), c.mean = mean(closure_duration), sum = sum(v.mean + c.mean))
mean_duration
```

```{r}
select(mean_duration, -sum) %>% gather("segment", "mean", v.mean, c.mean) %>% unite("voicing", c2_phonation, segment) %>% spread(voicing, mean) %>% summarise(v.diff = round(voiced_v.mean - voiceless_v.mean), c.diff = round(voiceless_c.mean - voiced_c.mean))
```

```{r}
med_duration <- group_by(durations_filtered, language, c2_phonation) %>% summarise(v.med = median(vowel_duration), c.med = median(closure_duration), sum = sum(v.med + c.med))
med_duration
```

```{r}
select(med_duration, -sum) %>% gather("segment", "med", v.med, c.med) %>% unite("voicing", c2_phonation, segment) %>% spread(voicing, med) %>% summarise(v.diff = round(voiced_v.med - voiceless_v.med), c.diff = round(voiceless_c.med - voiced_c.med))
```
...
```{r}
durations_2 %>%
  ggplot(aes(c1_phonation, vowel_duration)) +
  geom_boxplot() +
