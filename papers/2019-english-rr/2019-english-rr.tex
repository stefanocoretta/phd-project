...
words where the second consonant is voiced (for example,
/pada/).\footnote{Note that the release-to-release interval is not equivalent to the VC interval, since in the latter the VOT from the consonant preceding the vowel is excluded. See \citet{coretta2019k} for further details.}
...
determine the respective durations of vowel and consonant closure. Since
the VC boundary in voiceless stops is timed earlier within the
release-to-release relative to voiced stops, vowels are shorter and
closure are longer in the voiceless context than in the voiced context.
This agrees with the known differences of closure durations in voiceless
vs.~voiced stops
...
together. This same mechanism also gives rise to the so-called C-centre
effects observed with onsets, by which the acoustic duration of a vowel
...
Under the scenario presented in this paper, the combined action of the
isochrony of the vowel-to-vowel interval and the in-phase alignment of
the onset consonant is responsible for the isochrony of the
...
each other. The differential duration found in the voicing effect would
then be a consequence of the different velocity of the closing gesture
...
articulators \citep{de-jong1991}. Neither of this options were available
for this study due to time and resourses constraints. Second, as
discussed in \citet{coretta2019k}, less is known about the interplay
between segment durations, speech rate, and the vowel/closure relation,
making interpretation of data on correlation difficult. A third reason
for not correlating vowel and closure durations is that the main focus
of this study was to assess word-level gestural phasing via acoustic
measures. Future work will have to be carried out in which the
...
\citet{coretta2018j} were chosen for the intercept and the effect of C2
...
on results from \citet{coretta2018j}. For the random effects, a half
