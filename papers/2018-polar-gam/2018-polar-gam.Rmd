...
This 'echo' is captured by the transducer and translated into an image like the one shown in \Cref{f:uti} (for a technical description, see \citet{stone2005}).
...
These smoothing artefacts arise because, especially at the right-edge of these particular contours, the slope of the curve increases in such a way that the curve bends under itself (see for example the context /ug/, when *x* is between -30 and -20).
Since those points on the bend share the *x* value, the smooth just averages across the *y* values of those points.
\Cref{f:paths} shows a more appropriate (artefact-free) way of representing individual tongue contours.
...
The participants read nonce words embedded in the frame sentence *Dico \_\_ lentamente* 'I say \_\_ slowly'.
...
The output of `plot_polar_smooths()` is shown in \Cref{f:it01-gam-plot}.
...
Based on the predicted smooths sown in \Cref{f:it01-gam-plot}, we can argue that, in the context of coronal consonants, the root is more advanced in voiced relative to voiceless stops (when the vowel is either /a/ or /u/), and that the dorsum is also somewhat retracted in voiced stops if the vowel is /u/.
...
\Cref{f:it01-acf} show the autocorrelations in the residuals without (left) and with (right) an AR1 model.
...
Among the languages reported to show tongue root position differences between phonation categories in stops there are English \citep{westbury1983, ahn2018}, Brazilian Portuguese \citep{ahn2018}, and Hindi \citet{ahn2016a}.
...
\Cref{f:tongues-it01} to \Cref{f:tongues-pl07} show an appreciable degree of variation across speakers and phonological contexts in relation to the differences in tongue shapes between voiceless and voiced stops (IT07 and Pl05 both miss data from /u/ due to the poor quality of the ultrasonic image for this vowel).
...
Moreover, IT11 has the opposite patter, especially with velar stops, and voiced stops have a retracted tongue root compared to voiceless stops.
...
```{r tongues-it07, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in IT07.", fig.lp="f:", out.width=".8\\textwidth"}
plot_polar_smooths(it07_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r tongues-it11, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in IT11.", fig.lp="f:", out.width=".8\\textwidth"}
plot_polar_smooths(it11_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r tongues-pl02, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL02.", fig.lp="f:", out.width=".8\\textwidth"}
plot_polar_smooths(pl02_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r tongues-pl03, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL03.", fig.lp="f:", out.width=".8\\textwidth"}
plot_polar_smooths(pl03_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r tongues-pl04, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL04.", fig.lp="f:", out.width=".8\\textwidth"}
plot_polar_smooths(pl04_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r tongues-pl05, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL05.", fig.lp="f:", out.width=".8\\textwidth"}
plot_polar_smooths(pl05_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r tongues-pl06, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL06.", fig.lp="f:", out.width=".8\\textwidth"}
plot_polar_smooths(pl06_gam_ar, X, voicing, facet_terms = c2_place + vowel, split = list(vc_voicing = c("vowel", "c2_place", "voicing"))) +
  theme(legend.position = "top") +
  labs(x = "", y = "")
```

```{r tongues-pl07, include=TRUE, fig.cap = "Tongue contours of voiceless and voiced stops in PL07.", fig.lp="f:", out.width=".8\\textwidth"}
