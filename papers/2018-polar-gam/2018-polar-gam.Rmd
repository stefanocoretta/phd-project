...
The output of `plot_polar_smooths()` is shown in \Cref{f:Figure3}.
...
\Cref{f:Figure4} illustrates the use of difference smooths with the difference smooths of voiceless vs. voiced coronal stops when the vocalic context is /a/ or /u/.
...
Based on the predicted smooths sown in \Cref{f:Figure3}, we can argue that, in the context of coronal consonants, the root is more advanced in voiced relative to voiceless stops (when the vowel is either /a/ or /u/), and that the dorsum is also somewhat retracted in voiced stops if the vowel is /u/.
