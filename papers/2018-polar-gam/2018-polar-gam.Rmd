...
In the first case, the coordinates are transformed into polar before fitting.
The function `plot_polar_smooths()`, used for plotting the estimated contours, converts the coordinates back into cartesian.

<!-- brief intro on GAM in R (smooths, reference smooths etc, but for a more detailed overview, see...) -->
...
`vc_voicing` is an ordered factor which specifies for each contour the combination of C2 place, vowel, and voicing. The following code fits the specified model to the contour data of IT01. When running the code, the coordinates of the estimated origin used for the conversion to polar coordinates are returned.
The author can optionally specify those manually, for example when the contours are not in a fan-like system (like the data exported from AAA).
...
it01_gam_0 <- polar_gam(
  Y ~
    # remove parametric term
    vc_voicing +
    # reference smooth
    s(X),
    # remove difference smooth
    # s(X, by = vc_voicing),
  data = tongue_it01,
  method = "ML"
)
