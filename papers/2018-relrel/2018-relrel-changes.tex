...
\begin{abstract}
  ...
  While other factors (like perceptual biases \DIFaddbegin \DIFadd{and laryngeal effects}\DIFaddend ) could also play a role in the development of the voicing effect, the data discussed here shed new light on a possible production account of voicing-related differences in vowel durations.
\end{abstract}
...
see \citealt[Section V]{begus2017}\DIFaddbegin \DIFadd{, and \mbox{%DIFAUXCMD
\Cref{s:comp} }\hspace{0pt}%DIFAUXCMD
of this paper}\DIFaddend ).
...
compensatory temporal adjustments are plausible \DIFdelbegin \DIFdel{on the }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend light of the reviewed
...
the effect of stop voicing on vowel duration. On the other hand, \DIFdelbegin \DIFdel{they
...
voiced consonants, \mbox{%DIFAUXCMD
\citealt{beckman2013}}\hspace{0pt}%DIFAUXCMD
).
}\footnote{\DIFdel{Note that, while Polish neutralises the voicing contrast word-finally, it is maintained word-medially \mbox{%DIFAUXCMD
\citep{strycharczuk2012a}}\hspace{0pt}%DIFAUXCMD
.}}
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{For this reason, these two languages offer the opportunity to
investigate differences that could reveal mechanisms underlying the voicing effect. Moreover, given that }\DIFdelend \DIFaddbegin \DIFadd{the
typical realisation of phonological voiced stops in these languages are
similar (but see \mbox{%DIFAUXCMD
\citealt{huszthy2016} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citealt{schwartz2018} }\hspace{0pt}%DIFAUXCMD
for a
phonological and phonetic discussion on laryngeal aspects of Italian and
Polish
respectively).}\footnote{\DIFadd{Polish neutralises the voicing contrast word-finally, although the contrast is maintained word-medially \mbox{%DIFAUXCMD
\citep{strycharczuk2012a}}\hspace{0pt}%DIFAUXCMD
.}}
\DIFadd{\mbox{%DIFAUXCMD
\citet{cyran2011} }\hspace{0pt}%DIFAUXCMD
argues for a distinction between voicing and
aspirating varieties of Polish, based on phonological arguments.
\mbox{%DIFAUXCMD
\citet{waniek2011}}\hspace{0pt}%DIFAUXCMD
, on the other hand, cautiously argues that a possible
change in progress in Polish is affecting the VOT values of voiceless
stops in pre-stressed position.
}

\DIFadd{The non-clear status of Polish laryngeal phonology/phonetics could be
seen as a hindrance affecting the comparison with Italian. However,
based on data from Italian, \mbox{%DIFAUXCMD
\citeauthor{kirby2016a} }\hspace{0pt}%DIFAUXCMD
propose that the
distinction between voicing and aspirating languages itself
\mbox{%DIFAUXCMD
\citep{beckman2013} }\hspace{0pt}%DIFAUXCMD
cannot be straightforwardly mapped onto phonetics,
...
phonological literature \mbox{%DIFAUXCMD
\citep[2409]{kirby2016a}}\hspace{0pt}%DIFAUXCMD
. Since this study
focusses on the effect of post-stressed stops on preceding vowel
durations, we believe that the comparison between }\DIFaddend Italian and Polish \DIFdelbegin \DIFdel{share---on a
general level---some }\DIFdelend \DIFaddbegin \DIFadd{is
...
some }\DIFaddend features of the segmental and prosodic make-up of their
phonological systems, the design of the experimental material and
comparison of the results were facilitated. \DIFaddbegin \DIFadd{For these reasons, these
...
Similarly, \citet{keating1984} reports \DIFdelbegin \DIFdel{no effect of voicing on }\DIFdelend \DIFaddbegin \DIFadd{a difference of 2 ms in }\DIFaddend the
duration of stressed vowels in disyllabic words from 24 speakers\DIFaddbegin \DIFadd{, which
the author argues to be non-significant}\DIFaddend . On the other hand,
...
The \DIFaddbegin \DIFadd{variety of results concerning the voicing effect in Polish could be
...
is likely that the failure to obtain significant }\emph{\DIFadd{p}}\DIFadd{-values in some
studies are due to low statistical power, rather than because of absence
of the effect (as also hinted in \mbox{%DIFAUXCMD
\citealt{begus2017}}\hspace{0pt}%DIFAUXCMD
, see arguments in
\mbox{%DIFAUXCMD
\citealt{roettger2019} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citealt{nicenboim2018a}}\hspace{0pt}%DIFAUXCMD
).
}

\DIFadd{The }\DIFaddend acoustic data from the study discussed here suggests that (1) a
...
running on a \DIFdelbegin \DIFdel{Hawlett-Packard }\DIFdelend \DIFaddbegin \DIFadd{Hewlett-Packard }\DIFaddend ProBook 6750b laptop with Microsoft Windows
...
\DIFaddbegin \label{s:procedure}

\DIFaddend The participant was asked to read the sentences with the target words
...
IT02, who repeated the 12 sentences 5 times), which yields \DIFdelbegin \DIFdel{to }\DIFdelend a grand total
of 1212 tokens (792 from Italian, 420 from Polish).
\DIFdelbegin \footnote{\DIFdel{IT01 and IT02 (the first two participants of this study) also read sentences with words starting with /b/, which were later excluded from the experimental design. The data from /b/-initial words are not included in the analysis reported in this paper.}}
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend
...
\DIFaddend Moreover IT12 and IT14 produced several tokens of voiceless stops with
...
entirely absent
\citep{sharf1964}.\DIFaddbegin \footnote{\DIFadd{A reviewer makes interesting phonological remarks. The presence of lenition and voicing of voiceless stops in some varieties of Italian and its absence in Polish could be related to differences in laryngeal phonology and prosodic structure between these languages, namely the absence of a feature }[\DIFadd{voice}] \DIFadd{in Italian and the absence of true trochees in Polish. This hypothesis is compatible with work by \mbox{%DIFAUXCMD
\citet{schwartz2018} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{schwartz2016}}\hspace{0pt}%DIFAUXCMD
...
v3.5\DIFdelbegin \DIFdel{.1 }\DIFdelend \DIFaddbegin \DIFadd{.2 }\DIFaddend \citep{r-core-team2018, wickham2017}.
...
extracted with the \texttt{effects} package v4\DIFdelbegin \DIFdel{.0-3 }\DIFdelend \DIFaddbegin \DIFadd{.1-0 }\DIFaddend \citep{fox2003}. All
...
\textit{p}-values'. \DIFaddbegin \DIFadd{Inspection of residual plots and QQ plots of the
...
Italian and Polish. \DIFdelbegin \DIFdel{Vowel }\DIFdelend \DIFaddbegin \DIFadd{Vowels }\DIFaddend tend to be longer when followed by a voiced
...
significant. Stop closure is \DIFdelbegin \DIFdel{16.5 }\DIFdelend \DIFaddbegin \DIFadd{17.5 }\DIFaddend ms shorter (SE = \DIFdelbegin \DIFdel{3}\DIFdelend \DIFaddbegin \DIFadd{4}\DIFaddend ) if the stop is
...
longer when followed by voiced \DIFdelbegin \DIFdel{then }\DIFdelend \DIFaddbegin \DIFadd{than }\DIFaddend when followed by voiceless stops
...
\includegraphics[width=\linewidth]{2018-relrel_files/figure-latex/Figure6} \caption{\DIFaddFL{By-speaker random coefficients and error bars for the effect of C2 voicing on vowel duration, extracted from a mixed-effect model (Section 3.1).}}\label{f:Figure6}
\end{figure}

\DIFadd{On the other hand, while the estimated differences in vowel durations
...
and language. \mbox{%DIFAUXCMD
\Cref{f:Figure6} }\hspace{0pt}%DIFAUXCMD
shows the random coefficients of the
effect of C2 voicing on vowel duration for the individual speakers,
extracted from the mixed-effects model presented in \mbox{%DIFAUXCMD
\Cref{s:vduration}}\hspace{0pt}%DIFAUXCMD
.
...
\mbox{%DIFAUXCMD
\citet{malisz2008}}\hspace{0pt}%DIFAUXCMD
. While large-scale studies could reveal clear
...
\DIFaddend \hypertarget{compensatory-temporal-adjustment}{%
\subsection{Compensatory temporal
adjustment}\label{compensatory-temporal-adjustment}}

\DIFaddbegin \label{s:comp}

\DIFaddend Vowels followed by voiced stops are long, while vowels followed by
...
should target the preceding stressed vowel, rather \DIFdelbegin \DIFdel{then }\DIFdelend \DIFaddbegin \DIFadd{than }\DIFaddend the following
...
\DIFdelbegin \DIFdel{It is worth noting,
...
effect (see \mbox{%DIFAUXCMD
\citealt{kingston1994}}\hspace{0pt}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citealt{port1982}}\hspace{0pt}%DIFAUXCMD
, and
\mbox{%DIFAUXCMD
\citealt{luce1985}}\hspace{0pt}%DIFAUXCMD
), and other perceptual explanations of the voicing
effect have been proposed in \mbox{%DIFAUXCMD
...
interpreted as a by-product of gestural phasing, rather \DIFdelbegin \DIFdel{then }\DIFdelend \DIFaddbegin \DIFadd{than }\DIFaddend a
...
\DIFadd{The non-exclusivity of the release to release interval is also shown by
...
\mbox{%DIFAUXCMD
\citealt{bonett2006}}\hspace{0pt}%DIFAUXCMD
).}\footnote{\DIFadd{The CV of the release to release duration is 0.203, while that of the vowel onset to release duration is 0.232. The CQV is 0.127 for the release to release and 0.136 for the vowel onset to release. Lower values mean less dispersion/more cohesion.}}
\DIFadd{On the other hand, the duration of the interval between the vowel onset
...
\DIFadd{It could be argued that the temporal stability of the release to release
...
. Under this account, the stability of the release to
release interval could follow from the relative phasing of the vowels in
CVCV words (see Figure 6 in \mbox{%DIFAUXCMD
\citealt{tilsen2013}}\hspace{0pt}%DIFAUXCMD
...
. }\DIFaddend While beyond the scope of
this paper, \DIFaddbegin \DIFadd{articulatory }\DIFaddend work on the gestural coordination of sequences
besides the traditional syllable might reveal a principled organisation\DIFaddbegin \DIFadd{,
}\DIFaddend that results in the temporal patterns observed in this study \DIFaddbegin \DIFadd{and in
other durational phenomena}\DIFaddend .

\DIFaddbegin \DIFadd{Finally, it is worth noting that other accounts---which could be
...
in the enhancement of the effect (see \mbox{%DIFAUXCMD
\citealt{kingston1994}}\hspace{0pt}%DIFAUXCMD
,
\mbox{%DIFAUXCMD
\citealt{port1982}}\hspace{0pt}%DIFAUXCMD
...
when included in the model discussed in section \mbox{%DIFAUXCMD
\Cref{s:vcduration}}\hspace{0pt}%DIFAUXCMD
,
which has vowel duration as outcome and C2 closure duration as one of
the
predictors.}\footnote{\DIFadd{Multicollinearity is not an issue here, since the VIFs are all below 3 \mbox{%DIFAUXCMD
\citep{zuur2010}}\hspace{0pt}%DIFAUXCMD
.}}

\DIFadd{Even when multicollinearity between predictors is minimal, statistical
...
\mbox{%DIFAUXCMD
\citealt{mcelreath2015}}\hspace{0pt}%DIFAUXCMD
). For example, it is possible that, through
...
\mbox{%DIFAUXCMD
\citealt{johnson1997, soskuthy2018, ambridge2018, todd2018}}\hspace{0pt}%DIFAUXCMD
). Since
...
\DIFaddend \hypertarget{limitations-and-future-work}{%
...
\DIFaddbegin \DIFadd{Some concern could be raised in relation to possible influences of
...
\Cref{s:procedure}}\hspace{0pt}%DIFAUXCMD
, conversations during the
session prior to the experiment and instructions were in the
participant's native language. \mbox{%DIFAUXCMD
\citet{antoniou2010} }\hspace{0pt}%DIFAUXCMD
show that, in a
...
persist in proficient L2 speakers, as found by \mbox{%DIFAUXCMD
\citet{schwartz2015}}\hspace{0pt}%DIFAUXCMD
. The
...
\DIFaddend The compensatory temporal adjustment account presented here extends to
...
The results of \DIFdelbegin \DIFdel{an exploratory study on }\DIFdelend \DIFaddbegin \DIFadd{this exploratory study of }\DIFaddend the effect of voicing on vowel
...
voiced stops, \DIFdelbegin \DIFdel{causes }\DIFdelend \DIFaddbegin \DIFadd{can cause }\DIFaddend vowels to be shorter when followed by voiceless
...
\Cref{s:comp}}\hspace{0pt}%DIFAUXCMD
, the temporal patterns reported here do
...
(for a review of explanatory
pluralism in the cognitive sciences, see \mbox{%DIFAUXCMD
\citealt{dale2009} }\hspace{0pt}%DIFAUXCMD
and
...
obtained so far. }\DIFaddend Future work will need to investigate further aspects of
...
contrasts on the release to release interval\DIFaddbegin \DIFadd{, and in relation to other
attributes of consonant effects on vowel duration}\DIFaddend .
