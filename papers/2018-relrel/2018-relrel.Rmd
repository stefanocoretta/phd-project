...
@begus2017 argues that these findings support temporal compensation (although not univocally, see \citealt[Section V]{begus2017}).
...
Moreover, given that Italian and Polish share---on a general level---some features of the segmental and prosodic make-up of their phonological systems, the design of the experimental material and comparison of the results were facilitated.

Note that the distinction between voicing and aspirating varieties of Polish discussed in \citet{cyran2011} is based on phonological arguments rather than empirical data of VOT duration.
\citet{waniek2011}, on the other hand, cautiously argues that a possible change in progress in Polish is affecting the VOT of voiceless stops in pre-stressed position.
A reviewer points out that the non-clear status of Polish as a voicing language could interfere with the comparison.
However, based on data from Italian, \citeauthor{kirby2016a} propose that the distinction between voicing and aspirating languages itself cannot be straightforwardly mapped onto phonetics, and they remind us that 'the production of laryngeal contrasts of all kinds are considerably more complex' than generally described in the phonological literature \citep[2409]{kirby2016a}.
Moreover, since this study focusses on the effect of post-stressed stops on preceding vowel durations, we believe that the comparison between Italian and Polish is still feasible, even in the case Polish voiceless pre-stressed stops are articulated with longer VOT values.
...
These tokens have been used in the analyses, because (1) the actual presence or absence of voicing during closure does not bear on the compensatory account discussed here (which concerns supraglottal gestures) and laryngeal gestures can be implemented almost entirely independently from oral gestures, and (2) the voicing effect has been shown to exist even in whispered speech, where vocal fold vibration is entirely absent [@sharf1964].\footnote{A reviewer makes an interesting phonological remark. The presence of lenition and voicing of voiceless stops in Italian and its absence in Polish could be related to differences in prosodic structure between these languages, namely the absence of true trochees in Polish. This hypothesis is compatible with work by \citet{schwartz2016,schwartz2018}, to which the reader is referred.}
...
```{r Figure6, include=TRUE, fig.cap = "By-speaker random coefficients and error bars for the effect of C2 voicing on vowel duration, extracted from a mixed-effect model (Section 3.1).", fig.lp="f:", out.extra="width=\\linewidth", cache=TRUE, dependson = "vow-lm"}
...
Finally, while the estimated differences in vowel durations can be interpreted in reference to Italian and Polish as two independent linguistic objects, the patterns observed in the individual speakers does not indicate a systematic relation between magnitude of the effect and language.
\Cref{f:Figure6} shows the random coefficients of the effect of C2 voicing on vowel duration for the individual speakers, extracted from the mixed-effect model presented in \Cref{s:vduration}.
...
Under this account, the stability of the release to release interval could follow from the relative phasing of the two vowels in CVCV words (see Figure 6 in \citealt{tilsen2013}).
The stability of the temporal distance between vowels in Italian is preliminarily supported by acoustic data \citet{celata2014}.
While beyond the scope of this paper, articulatory work on the gestural coordination of sequences besides the traditional syllable might reveal a principled organisation, that results in the temporal patterns observed in this study and other durational phenomena.
...
Moreover, \citet{begus2017} finds that even when C2 closure duration is controlled for, C2 phonation (ejective, voiceless, voiced) is still a significant predictor.
Beguš argues for a separate laryngeal effect, which operates in addition to the closure duration effect.
In the present study, C2 voicing (voiceless, voiced) and its interactions are not significant when included in the model with vowel duration as outcome and, among other terms, C2 closure duration as predictor, discussed in section \Cref{s:vcduration}.\footnote{Multicollinearity is not an issue here, since the VIFs were all below 3 \citep{zuur2010}.}
...
The five Polish speakers with a highly proficient level of English investigated in this study showed a 10 ms increase in VOT values compared to the quasi-monolingual base level.
