...
An earlier VC boundary relative to the onset of the preceding vowel (like in the case of voiceless stops) causes the vowel to be shorter and the closure to be longer.
On the other hand, a later VC boundary (like with voiced stops) produces a longer vowel and a shorter closure.
...
Since the release to release interval includes just the vocoid gesture between the release of C1 and the VC boundary, and the consonant closure, it follows that differences in the timing of the VC boundary must be reflected in correlated differences in vowel and closure durations.
...
However, even if independent reasons for the interval stability can be identified, other mechanisms, unrelated to compensatory effects, would still be required to explain the differential timing of the VC boundary within that interval, so that accounts compatible with other aspects of production and perception would not be ultimately ruled out, as thoroughly discussed in \citet{begus2017}.
For example, the laryngeal adjustment hypothesis \citet{halle1967a} states that adjustments of the glottis for obstruent voicing require more time to be implemented, so that stop closure for a voiced stops is achieved later than that of a voiceless stop, relative to the onset of the preceding vowel.
...
Another account \citep{chen1970} makes direct reference to velocity of the closing gesture, which is faster in voiceless than in voiced stops \citet{van-summers1987, de-jong1991}, so that the VC boundary within the release-to-release would be timed earlier in the former than in the latter case.
Moreover, perceptual explanations of the voicing effect have been proposed in \citet{javkin1976} and \citet{kluender1988}, and these perceptual factors might play a role in the enhancement of the effect (see \citealt{kingston1994}, \citealt{port1982}, and \citealt{luce1985}).
...
The difference in results could be related to differences in the stimuli make-up (disyllabic vs. monosyllabic words), and more studies are needed that directly compare disyllabic and monosyllabic words.

<!-- Even when multicollinearity between predictors is minimal, statistical significance or non-significance of multiple terms cannot unequivocally inform us on the actual contribution of those terms, since it is possible that unknown relations, whether synchronic or diachronic, between terms mask underlying mechanisms (for a discussion see \citealt{mcelreath2015}).
On the one hand, it is possible that the original effect on vowel duration was driven by the laryngeal features and that a separate closure effect emerges in later stages via development of sub-distributions \citep{soskuthy2013}.
On the other hand, the reverse scenario is equally possible, in which the closure effect diachronically emerged first, with the laryngeal features effect subsequently deriving from it. -->
...
Such a pluralist view has already been proposed, for example, for incomplete neutralisation in \citealt{winter2011} (for a review of explanatory pluralism in the cognitive sciences, see \citealt{dale2009} and references therein).
