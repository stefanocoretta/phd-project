...
\begin{abstract}
  ...
  The durational difference of the first vowel \DIFdelbegin \DIFdel{then would }\DIFdelend \DIFaddbegin \DIFadd{and the stop closure would then }\DIFaddend follow from differences in \DIFdelbegin \DIFdel{closure durations of the following stop}\DIFdelend \DIFaddbegin \DIFadd{timing of the VC boundary within this interval}\DIFaddend .
  While other \DIFdelbegin \DIFdel{factors (like perceptual biases and laryngeal effects) could also play a role in the development of the voicing effect}\DIFdelend \DIFaddbegin \DIFadd{aspects, like production mechanisms related to laryngeal features effects and perceptual biases cannot be ruled out}\DIFaddend , the data discussed here \DIFdelbegin \DIFdel{sheds new light on a possible production account of voicing-related differences in vowel durations}\DIFdelend \DIFaddbegin \DIFadd{are compatible with a production account based on compensatory mechanisms}\DIFaddend .
\end{abstract}
...
is not affected by stop voicing. The duration of such \DIFaddbegin \DIFadd{a }\DIFaddend unit is held
...
\DIFdelbegin \DIFdel{Moreover, I would like to add that, if misplacement of the V-C boundary
...
misplacements would cancel each other out. }\DIFdelend \citet{begus2017} argues that these findings support temporal
...
According to such \DIFaddbegin \DIFadd{an }\DIFaddend account, the difference in vowel duration before
...
\DIFdelbegin \subsection{\DIFdel{The present study}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{the-present-study}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{the-present-study}{%
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{method}{%
\section{Method}\label{method}}
\DIFaddend

\label{s:method}

\DIFdelbegin \subsection{\DIFdel{Participants}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{participants}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{participants}{%
...
\DIFdelbegin \subsection{\DIFdel{Equipment}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{equipment}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{equipment}{%
...
\DIFdelbegin \subsection{\DIFdel{Materials}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{materials}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{materials}{%
...
V\textsubscript{1} (e.g.\DIFaddbegin \DIFadd{~}\DIFaddend /pata/, /pada/, /poto/,
...
\DIFdelbegin \subsection{\DIFdel{Procedure}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{procedure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{procedure}{%
...
\DIFdelbegin \subsection{\DIFdel{Data processing and
measurements}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{data-processing-and-measurements}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{data-processing-and-measurements}{%
...
\DIFdelbegin \subsection{\DIFdel{Statistical analysis}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{statistical-analysis}
%DIFDELCMD < %%%
...
levels: voiceless (vs.~voiced), /a/ (vs.\DIFaddbegin \DIFadd{~}\DIFaddend /o/, /u/), coronal (vs.~velar),
...
\DIFdelbegin \subsection{\DIFdel{Open Science statement}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{open-science-statement}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{open-science-statement}{%
...
Framework at \url{http://bit.ly/2FItZe1}.
...
\DIFdelbegin \subsection{\DIFdel{Vowel duration}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{vowel-duration}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{vowel-duration}{%
...
\DIFdelbegin \subsection{\DIFdel{Consonant closure
duration}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{consonant-closure-duration}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{consonant-closure-duration}{%
...
voiceless stops. The closure of \DIFdelbegin \DIFdel{voiceless }\DIFdelend \DIFaddbegin \DIFadd{voiced }\DIFaddend stops in Italian is 106.16 ms
long (SD = 27.08), while the \DIFdelbegin \DIFdel{voiced }\DIFdelend \DIFaddbegin \DIFadd{voiceless }\DIFaddend stops have a mean closure
duration of 117.66 ms (SD = 34.63). In Polish, the closure duration is
75.57 ms (SD = 16.16) in \DIFdelbegin \DIFdel{voiceless }\DIFdelend \DIFaddbegin \DIFadd{voiced }\DIFaddend stops and 83.11 ms (SD = 19.37) in
\DIFdelbegin \DIFdel{voiced
}\DIFdelend \DIFaddbegin \DIFadd{voiceless }\DIFaddend stops. The difference in closure duration based on the raw
...
\DIFdelbegin \subsection{\DIFdel{Vowel and closure
duration}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{vowel-and-closure-duration}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{vowel-and-closure-duration}{%
...
duration. However, such \DIFaddbegin \DIFadd{a }\DIFaddend correlation is quite weak, as shown by the
...
20 and 45\%. \DIFaddbegin \DIFadd{Note, moreover, that the negative correlation found here
could be a consequence of annotation bias, since the vowel and closure
share a boundary. }\DIFaddend Faster speech rates elicit a bigger effect than lower
...
\DIFdelbegin \subsection{\DIFdel{Word duration}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{word-duration}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{word-duration}{%
...
\DIFdelbegin \subsection{\DIFdel{Release to release interval
duration}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{release-to-release-interval-duration}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{release-to-release-interval-duration}{%
...
look for a possible source of such \DIFaddbegin \DIFadd{an }\DIFaddend effect in speech production. Only
...
\DIFdelbegin \subsection{\DIFdel{Voicing effect in Italian and
Polish}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{voicing-effect-in-italian-and-polish}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{voicing-effect-in-italian-and-polish}{%
...
\DIFdelbegin \DIFdel{it }\DIFdelend can be seen, speakers of both languages are scattered along the values
...
\DIFdelbegin \subsection{\DIFdel{Compensatory temporal
adjustment}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{compensatory-temporal-adjustment}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{compensatory-temporal-adjustment}{%
...
vowel duration \DIFaddbegin \DIFadd{and closure durations }\DIFaddend before voiceless vs.~voiced stops
can be seen as the outcome of differences in \DIFdelbegin \DIFdel{stop closure duration}\DIFdelend \DIFaddbegin \DIFadd{timing of the vowel
offset/closure onset (more neutrally, the VC boundary)}\DIFaddend . In other words,
the timing of the \DIFdelbegin \DIFdel{(acoustic) closure onset of C2 }\DIFdelend \DIFaddbegin \DIFadd{VC boundary }\DIFaddend within the temporally stable release to
release interval determines the duration of \DIFdelbegin \DIFdel{the
preceding vowel }\DIFdelend \DIFaddbegin \DIFadd{both the vowel and the stop
closure}\DIFaddend . An earlier \DIFdelbegin \DIFdel{closure onset }\DIFdelend \DIFaddbegin \DIFadd{VC boundary }\DIFaddend relative to the onset of the preceding
vowel \DIFdelbegin \DIFdel{(like in the case of voiceless stops) causes the vowel
to be shorter }\DIFdelend \DIFaddbegin \DIFadd{results in a shorter vowel and a longer stop closure}\DIFaddend . On the other
hand, a later \DIFdelbegin \DIFdel{closure onset (like with
voiced stops) }\DIFdelend \DIFaddbegin \DIFadd{VC boundary }\DIFaddend produces a longer vowel \DIFaddbegin \DIFadd{and a shorter closure}\DIFaddend .
\Cref{f:compensatory} illustrates \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend compensatory mechanism. Note that
...
C1 and the \DIFdelbegin \DIFdel{onset of the closure of C2 }\DIFdelend \DIFaddbegin \DIFadd{VC boundary, }\DIFaddend and the consonant closure, it follows that
differences in \DIFdelbegin \DIFdel{closure duration }\DIFdelend \DIFaddbegin \DIFadd{the timing of the VC boundary }\DIFaddend must be reflected in
differences in \DIFdelbegin \DIFdel{the duration of the preceding vocoid}\DIFdelend \DIFaddbegin \DIFadd{both vowel and closure durations}\DIFaddend .

Under an account of temporal compensation, the voicing effect can be
interpreted as a by-product of gestural phasing \DIFdelbegin \DIFdel{, rather than a
consequence of intrinsic features of voicing }\emph{\DIFdel{per se}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{and mechanisms operating
on the timing of the VC boundary}\DIFaddend . The temporal stability of the release
...
should not necessarily have a special status in such \DIFaddbegin \DIFadd{a }\DIFaddend compensatory
...
It \DIFdelbegin \DIFdel{could be argued }\DIFdelend \DIFaddbegin \DIFadd{is possible }\DIFaddend that the temporal stability of the release to release
interval \DIFdelbegin \DIFdel{could be the result of lengthening vowels followed by voiced
stops which have shorter closure durations(or, vice versa, of shortening vowels followed by voiceless stops), so that such stability
would not necessarily follow from }\DIFdelend \DIFaddbegin \DIFadd{is not an antecedent, but rather a consequence of manipulating
vowel and closure durations. If this were the case, the differential
duration of vowels and closures would not be the result of }\DIFaddend a
compensatory mechanism. \DIFdelbegin \DIFdel{However, what it is claimed here is that the stability of the interval is a
...
\mbox{%DIFAUXCMD
\citet{tilsen2013}}\hspace{0pt}%DIFAUXCMD
. Under this account, the }\DIFdelend \DIFaddbegin \DIFadd{The present data cannot disambiguate between
...
is promising, in that the temporal
}\DIFaddend stability of the release to release interval \DIFdelbegin \DIFdel{could }\DIFdelend \DIFaddbegin \DIFadd{would directly }\DIFaddend follow from
the relative phasing of the vowels in CVCV words (see Figure 6 in
\citealt{tilsen2013}). \DIFdelbegin \DIFdel{The stability of the
temporal distance between vowels in Italian is preliminarily supported
by the acoustic data in \mbox{%DIFAUXCMD
\citet{celata2014}}\hspace{0pt}%DIFAUXCMD
. While beyond the scope of
this paper, articulatory }\DIFdelend \DIFaddbegin \DIFadd{Articulatory }\DIFaddend work on the gestural coordination of
sequences besides the traditional syllable might reveal a principled
organisation \DIFdelbegin \DIFdel{,
}\DIFdelend that results in the temporal patterns observed in this
study and in other durational phenomena.

\DIFdelbegin \DIFdel{Finally, it is worth noting that other accounts---which could be }\DIFdelend \DIFaddbegin \DIFadd{However, even if independent reasons for the interval stability can be
...
within that interval. Accounts }\DIFaddend compatible with other aspects of
production and \DIFdelbegin \DIFdel{perception---aren't
}\DIFdelend \DIFaddbegin \DIFadd{perception would not be }\DIFaddend ultimately ruled out\DIFaddbegin \DIFadd{, as
thoroughly discussed in \mbox{%DIFAUXCMD
\citet{begus2017}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . For example, \DIFdelbegin \DIFdel{perceptual }\DIFdelend \DIFaddbegin \DIFadd{the laryngeal
adjustment hypothesis \mbox{%DIFAUXCMD
\citep{halle1967a} }\hspace{0pt}%DIFAUXCMD
states that adjustments of the
glottis for obstruent voicing require more time to be implemented, so
that stop closure for a voiced stops will be achieved later than that of
a voiceless stop, relative to the onset of the preceding vowel. Tongue
root advancement \mbox{%DIFAUXCMD
\citep{rothenberg1967, westbury1983, ohala2011} }\hspace{0pt}%DIFAUXCMD
could
also play a role in modulating the time required before closure can be
implemented. Another account \mbox{%DIFAUXCMD
\citep{chen1970} }\hspace{0pt}%DIFAUXCMD
makes direct reference to
velocity of the closing gesture, which is faster in voiceless than in
voiced stops \mbox{%DIFAUXCMD
\citep{van-summers1987, de-jong1991}}\hspace{0pt}%DIFAUXCMD
, so that the VC
boundary within the release to release would be timed earlier in the
former than in the latter case. Moreover, perceptual explanations of the
voicing effect have been proposed in \mbox{%DIFAUXCMD
\citet{javkin1976} }\hspace{0pt}%DIFAUXCMD
and
\mbox{%DIFAUXCMD
\citet{kluender1988}}\hspace{0pt}%DIFAUXCMD
, and these perceptual }\DIFaddend factors might play a role in
the enhancement of the effect (see \citealt{kingston1994},
\citealt{port1982}, and \citealt{luce1985}\DIFdelbegin \DIFdel{), and other perceptual
explanations of
the voicing effect have been proposed in \mbox{%DIFAUXCMD
\citet{javkin1976} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{kluender1988}}\hspace{0pt}%DIFAUXCMD
. Moreover, }\DIFdelend \DIFaddbegin \DIFadd{, see \mbox{%DIFAUXCMD
\citealt{fowler1992} }\hspace{0pt}%DIFAUXCMD
for
a critique to \mbox{%DIFAUXCMD
\citealt{kluender1988}}\hspace{0pt}%DIFAUXCMD
). Finally, whether the timing of
the VC boundary depends on modulations of the vocalic or consonantal
gesture, or both, is another aspect that should be investigated further
(see \mbox{%DIFAUXCMD
\citealt{de-jong1991} }\hspace{0pt}%DIFAUXCMD
for an example).
}

\DIFadd{A comment is also due in relation to possible coexisting effects on
vowel duration. }\DIFaddend \citet{begus2017} finds that\DIFaddbegin \DIFadd{, }\DIFaddend even when C2 closure
duration is controlled for, C2 phonation (ejective, voiceless, voiced)
in Georgian is still a significant predictor. \DIFdelbegin \DIFdel{Beguš }\DIFdelend \DIFaddbegin \DIFadd{The author }\DIFaddend argues for a
separate laryngeal \DIFaddbegin \DIFadd{features }\DIFaddend effect, which operates in addition to \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a
}\DIFaddend closure duration effect. In the present study, C2 voicing (voiceless,
...
%DIFDELCMD < %%%
\DIFdel{Even }\DIFdelend \DIFaddbegin \DIFadd{However, even }\DIFaddend when multicollinearity between predictors is minimal,
\DIFaddbegin \DIFadd{presence or lack of }\DIFaddend statistical significance of multiple terms cannot
...
mechanisms (for a discussion see \citealt{mcelreath2015}). \DIFdelbegin \DIFdel{For example}\DIFdelend \DIFaddbegin \DIFadd{The
diachronic development of context-driven statistical sub-distributions
can override the original causal link \mbox{%DIFAUXCMD
\citep{soskuthy2013}}\hspace{0pt}%DIFAUXCMD
. Under this
scenario}\DIFaddend , it is \DIFdelbegin \DIFdel{possible that, through
...
\mbox{%DIFAUXCMD
\citep{soskuthy2013}}\hspace{0pt}%DIFAUXCMD
. This can result in the ability for the phonation
predictor to capture variance in vowel durations, even if the original
mechanism directly involved closure durations only }\DIFdelend \DIFaddbegin \DIFadd{not possible to discern which of the competing
...
had a role in generating the synchronic patterns }\DIFaddend (this kind of reasoning
is compatible \DIFaddbegin \DIFadd{for example }\DIFaddend with exemplar theories of speech perception
and production, see among others
\citealt{johnson1997, soskuthy2018, ambridge2018, todd2018}).
\DIFdelbegin \DIFdel{Since
opposite }\DIFdelend \DIFaddbegin

\DIFadd{Since diverging }\DIFaddend results have been obtained in relation to the
significance of C2 phonation in addition to C2 closure durations, these
\DIFdelbegin \DIFdel{results }\DIFdelend \DIFaddbegin \DIFadd{aspects }\DIFaddend need to be \DIFdelbegin \DIFdel{replicated }\DIFdelend \DIFaddbegin \DIFadd{further investigated }\DIFaddend in future studies, although to
ascertain whether they are artefacts of statistical procedures or if
they reflect an underlying state of affairs might still prove difficult\DIFdelbegin \DIFdel{(cf.
~the correlation
vs.
~causation problem).
}\DIFdelend \DIFaddbegin \DIFadd{.
To conclude, lack of significance of a separate laryngeal effect cannot
be taken as evidence for its absence in the present data, and a
compensatory mechanism could coexist with mechanisms directly related to
laryngeal features, which would in turn explain the differential timing
of the VC boundary.
}\DIFaddend

\DIFdelbegin \subsection{\DIFdel{Limitations and future
work}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{limitations-and-future-work}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{limitations-and-future-work}{%
...
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{conclusions}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{conclusions}{%
...
could conspire together to produce the observed patterns. Such \DIFaddbegin \DIFadd{a
}\DIFaddend pluralist view has already been proposed \DIFdelbegin \DIFdel{, for example, }\DIFdelend for \DIFdelbegin \DIFdel{incomplete
neutralisation in \mbox{%DIFAUXCMD
\citealt{winter2011} }\hspace{0pt}%DIFAUXCMD
(for }\DIFdelend \DIFaddbegin \DIFadd{the voicing effect (for
example, \mbox{%DIFAUXCMD
\citealt{begus2017} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citealt{sanker2018}}\hspace{0pt}%DIFAUXCMD
), and for other
related phenomena, like vowel duration in incomplete neutralisation
\mbox{%DIFAUXCMD
\citep{winter2011}}\hspace{0pt}%DIFAUXCMD
. For }\DIFaddend a review of explanatory pluralism in the
cognitive sciences, see \citealt{dale2009} and references therein\DIFdelbegin \DIFdel{).
A hybrid accountwhich }\DIFdelend \DIFaddbegin \DIFadd{.
Indeed, a hybrid account, which takes into consideration and }\DIFaddend synthesises
aspects of multiple \DIFdelbegin \DIFdel{accounts }\DIFdelend \DIFaddbegin \DIFadd{proposed accounts, }\DIFaddend is probably warranted, given the
diversity of \DIFaddbegin \DIFadd{compatible }\DIFaddend results obtained so far. Future work will need
...
\DIFdelbegin \section{\DIFdel{Acknowledgements}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{acknowledgements}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{acknowledgements}{%
...
\DIFdelbegin \section{\DIFdel{Funding information}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{funding-information}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{funding-information}{%
...
\DIFdelbegin \section{\DIFdel{Competing interests}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{competing-interests}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{competing-interests}{%
...
\DIFdelbegin \section{\DIFdel{Output of statistical
models}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{output-of-statistical-models}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{output-of-statistical-models}{%
...
\DIFdelbegin \section{\DIFdel{Socio-linguistic information of
participants}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{socio-linguistic-information-of-participants}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{socio-linguistic-information-of-participants}{%
...
\DIFdelbegin \section{\DIFdel{Target words}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{target-words}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{target-words}{%
