...
shorter and the closure to be longer. On the other hand, a later VC
boundary (like with voiced stops) produces a longer vowel and a shorter
closure. \Cref{f:compensatory} illustrates this compensatory mechanism.
Note that the term `temporal stability' (and `temporally stable') as
used here means that the underlying statistical distribution of the
interval duration is stable \emph{across contexts of C2 voicing}. No
specific statement is implied about the variance of the duration around
the mean, across or within phonological contexts.
...
correlated differences in vowel and closure durations.
...
within that interval, so that accounts compatible with other aspects of
...
adjustment hypothesis \citet{halle1967a} states that adjustments of the
glottis for obstruent voicing require more time to be implemented, so
that stop closure for a voiced stops is achieved later than that of a
voiceless stop, relative to the onset of the preceding vowel. Tongue
...
voiced stops \citet{van-summers1987, de-jong1991}, so that the VC
...
\citealt{port1982}, and \citealt{luce1985}). Finally, whether the timing
of the VC boundary depends on modulations of the vocalic or consonantal
...
The difference in results could be related to differences in the stimuli
make-up (disyllabic vs.~monosyllabic words), and more studies are needed
that directly compare disyllabic and monosyllabic words.
