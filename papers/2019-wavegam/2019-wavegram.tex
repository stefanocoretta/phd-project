...
\begin{textblock}{3}(0.2,0.2)
  \textcolor{lightgray}{v1.0 (2019/05/27)}
\end{textblock}
\maketitle
...
pilot study which compares modal and breathy phonation, and the results
...
neck, one on each side of the thyroid cartilage (\Cref{f:egg-setup}).
...
as an index of phonation type. Modal voice has higher closed quotient
...
\Cref{s:pilot} presents results from a pilot study which informally
evaluates the proposed method are presented. \Cref{s:voicing}
illustrates how to conduct a wavegram GAM analysis of dEGG data through
a practical example in which the wavegrams of vowels followed by
voiceless and voiced stops in Italian and Polish are compared. This
analysis indicates the presence of a pattern of glottal spreading in the
second half of vowels followed by voiceless stops and of
cross-linguistic differences. Insights on the diachronic emergence of
pre-aspiration based on these results are discussed. Finally,
\Cref{s:conclusion} concludes and discusses the limitations of the
current implementation of the method and future directions.
