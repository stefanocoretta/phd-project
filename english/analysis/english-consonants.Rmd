...
clos_1 <- brm(
  ...
  prior = priors,
  cores = 4,
  file = "./english/analysis/cache/clos_1"
)
```
...
```{r clos-1-fixed}
clos_1_fixed <- tidy(clos_1, effects = "fixed", conf.level = 0.95, fix.intercept = FALSE) %>%
  mutate(
...
clos_1_fixed %>%
  mutate(
    theta = c(90, -20, 15, 0, -25, 0, 0, 0, 0),
    sigma_prior = c(20, 10, 10, 25, 10, 30, 30, 30, 30),
...
```{r clos-1-intervals}
clos_1_draws %>%
  filter(.variable %in% c("b_voicingvoiced", "b_num_sylmono", "b_voicingvoiced:num_sylmono", "b_placelabial")) %>%
