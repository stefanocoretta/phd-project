...
priors <- c(
  ...
  set_prior("normal(1.65, 2.8)", class = "b", coef = "c2_voicevoiced"),
  ...
  set_prior("cauchy(13.85, 2.1)", class = "sigma")
)
```
...
```{r}
sim_data_2 <- filter(sim_data, speaker %in% c("s01", "s02"))

pred_rr <- predict(rr_fit, newdata = sim_data_2, sample_new_levels = "gaussian", summary = FALSE, subset = 1, nsamples = 1)

pred_data <- cbind(pred_rr = as.vector(pred_rr), sim_data_2)
```

```{r}
rr_fit_2 <- update(rr_fit, newdata = pred_data, recompile = FALSE)
```
