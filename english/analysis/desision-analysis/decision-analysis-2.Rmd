...
```{r}
ggplot(simd, aes(c2_voice, rr)) + geom_boxplot() + facet_wrap(~ speaker)
```
...
# Bayesian credible intervals analysis
...
for (i in 1:speakers) {
  rr <- c(rr, rnorm(items / 2, 200, 10), rnorm(items / 2, 200, 10) + rnorm(1, 5, 1.25))
}
...
```{r}
simd_2 <- simd %>%
  filter(speaker %in% sample(levels(simd$speaker), 10))

rr_new <- update(rr_fit_1, newdata = simd_2)
...
```{r}
eng_x <- extend(eng_durations_rr, along = "speaker", n = 60)
