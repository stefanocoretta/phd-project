...
rr_1 <- brm(
  ...
  cores = 4,
  seed = 1234,
  file = "./english/analysis/cache/rr_1"
)
```
...
rr_2 <- brm(
  ...
  cores = 4,
  seed = 1234,
  file = "./english/analysis/cache/rr_2"
)
```
...
rr_3 <- brm(
  ...
  cores = 4,
  seed = 1234,
  file = "./english/analysis/cache/rr_3"
)
```
...
rr_3_fixed %>%
  mutate(
    theta = c(200, 0, 50, 0, 0, 0, -25, 50),
    sigma_prior = c(50, 25, 25, 30, 30, 30, 10, 25),
...
rr_4 <- brm(
  ...
  cores = 4,
  seed = 1234,
  file = "./english/analysis/cache/rr_4"
)
```

```{r rr-4-sum}
summary(rr_4, prob = 0.95)
```

```{r rr-4-ms}
as_tibble(fixef(rr_4)[-1,] * exp(fixef(rr_4))[1], rownames = "term") %>%
  mutate(ci_width = abs(Q2.5 - Q97.5))
```

```{r rr-4-pp-check}
pp_check(rr_4, nsamples = 50)
```
