...
voicing <- list.files(path = "pilot/results",
                   pattern = "*-voicing.csv",
                   full.names = TRUE) %>%
    map_df(~read_csv(., na = "--undefined--")) %>%
    left_join(., languages) %>%
...
voicing.voiceless <- filter(voicing, c2phonation == "voiceless")
...
plot(allEffects(model.closure.it)[3])
```
...
```{r voicing-duration}
ggplot(voicing.voiceless, aes(vowel, voicing)) +
    geom_boxplot() +
