...
splines.closure.it <- filter(closure, language == "italian")
splines.closure.pl <- filter(closure, language == "polish")
```
...
```{r max-it-gam-ar}
r1 <- start_value_rho(max.it.gam.2, plot = TRUE)
...
acf_resid(closure.it.gamm, split_pred=c("rec.date"))
```

### AR1 model

```{r closure-it-gam-ar}
r1 <- start_value_rho(closure.it.gam.2)
