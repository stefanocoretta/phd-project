...
degg_it <- list.files(path = "./italian-polish/results/egg",
...
ggplot(aes(x = rel.time * 1000, colour = c2_phonation_o)) +
    facet_grid(. ~ language + speaker) +
    geom_smooth(aes(y = maximum), size = 0.5, alpha = 0.2) +
    geom_smooth(aes(y = minimum), size = 0.5, alpha = 0.2) +
...
```{r cq-boxplot}
degg_it %>%
    group_by(speaker, c2_phonation_o, date) %>%
    summarise(closed_quotient = mean(closed_quotient)) %>%
ggplot(aes(x = c2_phonation_o, y = closed_quotient)) +
    facet_grid(. ~ speaker) +
    geom_boxplot() +
    xlab("C2 phonation") +
...
```{r tracegram-point-test}
degg_it %>%
    filter(speaker == "it02", word == "paca") %>%
ggplot(aes(x = proportion)) +
    facet_grid(. ~ language + speaker) +
    geom_point(aes(y = maximum), alpha = 0.5, colour = "red") +
    geom_point(aes(y = minimum), alpha = 0.5, colour = "blue") +
    xlab("proportion of vowel duration") +
    ylab("proportion of glottal period") +
    xlim(0.1, 0.9) +
    scale_color_discrete(name = "C2 voicing") +
