...
voicing <- list.files(path = "italian-polish/data/datasets/egg",
                   pattern = "*-voicing.csv",
                   full.names = TRUE) %>%
    map_df(~read_csv(., na = "--undefined--")) %>%
    left_join(., languages) %>%
...
voicing.voiceless <- filter(voicing, c1_phonation == "voiceless", c2_phonation == "voiceless") %>%
  group_by(speaker) %>%
  mutate(voicing_duration_z = scale(voicing_duration))
...
```{r word-plot-it}
ggplot(vowels.it, aes(c2_phonation, word_duration)) +
...
ggplot(vowels.it, aes(vowel, vowel.duration)) +
    geom_boxplot() +
    facet_grid(speaker ~ c2_place + c2_phonation)
```
...
```{r word-plot-pl}
ggplot(vowels.pl, aes(c2_phonation, word.duration)) +
    geom_violin() +
...
ggplot(filter(vowels.it, c2_place == "velar"), aes(c2_phonation, vowel.duration, colour = speaker)) +
    facet_grid(. ~ vowel) +
    geom_violin(position = dodge) +
    geom_boxplot(width=0.1, position = dodge)

ggplot(filter(vowels.it, c2_place == "coronal"), aes(c2_phonation, vowel.duration, colour = speaker)) +
    facet_grid(. ~ vowel) +
    geom_violin(position = dodge) +
    geom_boxplot(width=0.1, position = dodge)

ggplot(vowels.pl, aes(c2_phonation, vowel.duration)) +
    facet_grid(. ~ vowel + c2_place) +
    geom_violin() +
    geom_boxplot(width=0.2)
```
...
```{r proportions}
proportions <- vowels %>%
    group_by(c2_phonation, language) %>%
    summarise(
        total = mean(vowel_duration, na.rm = TRUE) +
            mean(closure_duration, na.rm = TRUE) +
            mean(rvot, na.rm = TRUE),
        vowel = mean(vowel_duration, na.rm = TRUE) / total,
        closure = mean(closure_duration, na.rm = TRUE) / total,
...
```{r plot-proportions}
ggplot(proportions, aes(c2_phonation, duration, fill = segment)) +
...
word.proportions <- vowels %>%
    group_by(c1_phonation, c2_phonation, language) %>%
    summarise(
        total = 
            mean(c1_closure, na.rm = TRUE) +
            mean(c1_rvot, na.rm = TRUE) +
            mean(vowel_duration, na.rm = TRUE) +
            mean(closure_duration, na.rm = TRUE) +
            mean(rvot, na.rm = TRUE) +
            mean(v2_duration, na.rm = TRUE),
        c1_closure = mean(c1_closure, na.rm = TRUE),
        c1_rvot = mean(c1_rvot, na.rm = TRUE),
        vowel = mean(vowel_duration, na.rm = TRUE),
        closure = mean(closure_duration, na.rm = TRUE),
        rvot = mean(rvot, na.rm = TRUE),
        v2 = mean(v2_duration, na.rm = TRUE)
    ) %>%
    gather(segment, duration, c1_closure:v2) %>%
    mutate(
      segment = factor(
        segment,
        levels = c("v2", "rvot", "closure", "vowel", "c1_rvot", "c1_closure")
      )
    ) %>%
    unite(phonation, c1_phonation, c2_phonation)
```
...
```{r voicing-duration}
ggplot(voicing.voiceless, aes(vowel, voicing_duration)) +
    geom_violin() +
...
voicing.voiceless %>%
  ggplot(aes(language, voicing_duration, colour = vowel)) +
  geom_boxplot()
```
...
```{r}
voicing.voiceless %>%
  ggplot(aes(vowel, voicing_duration_z)) +
  geom_boxplot() +
  facet_grid(~ language)
```
...
```{r}
voicing.voiceless %>%
  ggplot(aes(voicing_duration_z, colour = vowel)) +
  geom_density() +
  facet_grid(~ language)
```
...
ggplot(voicing.voiceless.it, aes(closure.duration, voicing_duration, colour = speaker)) +
    geom_point() +
    geom_smooth(method = "lm")
```
...
ggplot(voicing.voiceless.pl, aes(closure.duration, voicing_duration, colour = speaker)) +
    geom_point() +
    geom_smooth(method = "lm")
```
...
```{r}
ggplot(voicing.gestures, aes(language, VOM, colour = c2_phonation)) +
    geom_boxplot() +
    facet_grid(vowel ~ c2_place)
```

```{r}
ggplot(voicing.gestures, aes(VOM, colour = c2_phonation)) +
    geom_density() +
    facet_grid(language ~ vowel + c2_place)
```
...
# Vowel duration and voicing of C1 and C2

```{r}
vowels_2 <- filter(vowels, speaker %in% c("it01", "it02")) %>%
  unite("c1c2_phonation", c1_phonation, c2_phonation, remove = FALSE) %>%
  mutate(c1c2_phonation = factor(c1c2_phonation, levels = c("voiceless_voiceless", "voiced_voiceless", "voiceless_voiced", "voiced_voiced")))
```
...
vowels_2 %>%
  ...
  ggplot(aes(c1c2_phonation, vowel_duration)) +
  geom_boxplot()
```
