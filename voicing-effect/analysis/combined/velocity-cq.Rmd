...
velocity <- list.files(path = "./voicing-effect/data/datasets/cached/",
                   pattern = "*-velocity-profiles.csv",
                   full.names = TRUE) %>%
...
velocity.coronal <- filter(velocity, c2_place == "coronal")
velocity.velar <- filter(velocity, c2_place == "velar")

degg.word <- list.files(path = "./voicing-effect/results",
...
ggplot(velocity.coronal, aes(rel.time, TT.velocity.abs, group = repetition)) +
    geom_line() +
    facet_grid(speaker ~ vowel + c2_phonation)
```
...
ggplot(velocity, aes(rel.time, TD.velocity.abs)) +
    geom_line() +
    facet_grid(speaker + c2_place ~ c2_phonation + vowel)
```
