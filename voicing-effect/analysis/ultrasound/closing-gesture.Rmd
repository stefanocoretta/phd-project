...
velocity <- list.files(
  ...
  group_by(speaker) %>%
  mutate(
    TT_velocity_abs_norm = scale(TT_velocity_abs),
    TD_velocity_abs_norm = scale(TD_velocity_abs)
  ) %>%
  left_join(y = languages) %>%
  left_join(y = words) %>%
  mutate_if(is.character, as.factor)
...
velocity %>%
  ...
  ggplot(aes(c2phonation, TT_velocity_abs_norm)) +
  geom_boxplot() +
  facet_grid(~ language)
```
...
```{r}
velocity %>%
  filter(label == "peak1_TT", language == "italian") %>%
  ggplot(aes(c2phonation, TT_velocity_abs_norm)) +
  geom_boxplot() +
  facet_grid(. ~ vowel)
```
...
```{r}
velocity %>%
  filter(label == "peak1_TD", language == "italian") %>%
  ggplot(aes(c2phonation, TD_velocity_abs_norm)) +
  geom_boxplot() +
  facet_grid(. ~ vowel)
```
