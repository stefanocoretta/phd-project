...
filter(splines.pol.it, label == "max_TT", vowel == "u") %>%
    ggplot(aes(Y, X, colour = c2_phonation)) +
...
filter(splines.pol.it, label == "max_TT") %>%
    ggplot(aes(Y.car, X.car, colour = c2_phonation, group = rec.date)) +
#    geom_line(stat = "smooth", method = "loess", alpha = 0.5) +
    geom_point(alpha = 0.5) +
    coord_fixed(ratio = 1) +
    facet_grid(vowel ~ .) +
...
splines.it %>%
    filter(label == "max_TT", speaker == "it01", vowel == "a") %>%
    ggplot(aes(X, Y)) +
    geom_point(aes(colour = c2_phonation)) +
...
splines.it %>%
    ...
    geom_point(aes(colour = c2_phonation)) +
    geom_smooth(aes(colour = c2_phonation))
```
...
it01 <- polar %>%
    mutate(
        ...
        vow_phon = ordered(interaction(vowel, c2_phonation), levels = c("a.voiceless", "a.voiced", "o.voiceless", "o.voiced", "u.voiceless", "u.voiced")),
        place_phon = ordered(interaction(c2_place, c2_phonation), levels = c("coronal.voiceless", "coronal.voiced", "velar.voiceless", "velar.voiced"))
    ) %>%
...
it01_gam <- bam(
    radius ~
        ...
        vowel +
        s(angle, bs = "cr") +
        s(angle, by = c2_phonation,  bs = "cr") +
        s(angle, by = c2_place,  bs = "cr") +
...
ggplot(polar_pred, aes(X, Y, colour = c2_phonation)) +
    geom_path() +
    facet_wrap(~ c2_place + vowel)
```
...
it01_cart <- splines.it %>%
    ...
    mutate(
        ...
        c2_place = ordered(c2_place, levels = c("coronal", "velar")),
        c2_phonation = ordered(c2_phonation, levels = c("voiceless", "voiced")),
        vow_phon = ordered(interaction(vowel, c2_phonation)),
        place_phon = ordered(interaction(c2_place, c2_phonation))
    ) %>%
    arrange(rec.date, fan_line) %>%
    create_event_start("rec.date") %>%
    droplevels()
...
ggplot(polar_m, aes(angle, radius, colour = c2_phonation)) +
    geom_point(alpha = 0.5)
```
...
ggplot(polar_r, aes(angle, radius, colour = c2_phonation)) +
    geom_point(alpha = 0.5)
```
...
ggplot(polar_all_r, aes(angle, radius, colour = c2_phonation)) +
    geom_point(alpha = 0.5) +
    geom_smooth() +
...
```{r}
cartesian_ci %>%
    ggplot(aes(CI_X, CI_Y, fill = c2_phonation)) +
    geom_polygon(alpha = 0.2) +
    geom_path(data = it01_gam_all_r_pred, aes(X, Y, colour = c2_phonation)) +
    facet_grid(c2_place ~ vowel) +
    coord_fixed(ratio = 1) +
    xlab("") +
    ylab("")
...
splines_it %>%
  filter(vowel == "a") %>%
  ggplot(aes(angle_norm, radius, colour = c2_phonation)) +
...
```{r}
splines_it %>%
  ggplot(aes(angle_norm, radius_norm, colour = c2_phonation)) +
  geom_smooth() +
  facet_grid(c2_place ~ speaker)
```
...
tongue_it_gam <- bam(
  radius ~
    ...
    vowel +
    s(angle_norm) +
    s(angle_norm, by = c2_phonation) +
    s(angle_norm, by = c2_place) +
...
```{r}
tongue_it_pred %>%
  ggplot(aes(X, Y, colour = c2_phonation, linetype = c2_phonation)) +
  geom_path() +
  facet_grid(c2_place ~ vowel)
```
