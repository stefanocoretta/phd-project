...
tongue %>%
    filter(gesture == "max") %>%
    ggplot(aes(X, Y, group = rec.date, colour = c2phonation)) +
    geom_path(alpha = 0.5) 
```
...
tongue.max <- tongue %>%
    ...
    mutate(
        ...
        place.vow.phon = interaction(c2place, vowel, c2phonation),
...
tongue.max.a <- tongue.max %>%
    filter(vowel == "a", c2place == "coronal")
```
...
#        s(theta, by = c2place.ord, bs = "cr") +
        s(theta, rec.date, bs = "fs", xt = "cr", m = 1, k = 5),
    data = filter(tongue.mm.5, c2place == "coronal"),
    method = "fREML"
)
...
plot_gamsd(
    ...
    "theta",
    list(c2phonation.ord = c("voiceless", "voiced")),
    list(vowel.ord = "a", c2place.ord = "coronal")
)
```
