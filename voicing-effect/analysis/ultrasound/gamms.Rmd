...
tongue <- read_aaa("italian-polish/results/ultrasound/it01-tongue-cart.tsv", columns) %>%
...
tongue.mm <- read_aaa("italian-polish/results/ultrasound/it01-tongue-pol-mm.tsv", columns, coordinates = "polar") %>%
    separate(label, c("gesture", "position")) %>%
    mutate(word = word(prompt, 2)) %>%
    left_join(y = languages) %>%
    left_join(y = words) %>%
...
tongue.mm.5 <- read_aaa("italian-polish/results/ultrasound/it05-tongue-pol-mm.tsv", columns, coordinates = "polar") %>%
    separate(label, c("gesture", "position")) %>%
    mutate(word = word(prompt, 2)) %>%
    left_join(y = languages) %>%
    left_join(y = words) %>%
...
```{r}
tongue.pol <- read_aaa("italian-polish/results/ultrasound/it01-tongue-pol.tsv", columns, coordinates = "polar") %>%
    separate(label, c("gesture", "position")) %>%
    mutate(word = word(prompt, 2)) %>%
    left_join(y = languages) %>%
    left_join(y = words) %>%
