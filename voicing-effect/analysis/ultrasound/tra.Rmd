...
tra_series <- list.files(
  ...
  mutate(
    ...
    vowel_phon = interaction(vowel, c2_phonation)
  ) %>%
...
contrasts(tra_series$vowel_phon) <- "contr.treatment"
...
```{r tra-boxplot-speakers-2-a}
tongue_root %>%
  filter(tra_z < 3, c2_place == "coronal", vowel == "a") %>%
  ggplot(aes(speaker, tra_z, fill = c2_phonation)) +
  geom_boxplot()
```
...
```{r tr-voicing-box}
tongue_root %>%
  ggplot(aes(speaker, tra, fill = c2_phonation)) +
  geom_boxplot()
```
...
tongue_root %>%
  filter(speaker == "it02") %>%
  ggplot(aes(c2_phonation, tra, fill = c2_phonation)) +
  geom_boxplot()
```

```{r tr-difference-voicing}
tongue_root %>%
  group_by(speaker, c2_phonation) %>%
  summarise(mean = mean(tra)) %>%
  spread(c2_phonation, mean) %>%
  mutate(difference = voiced - voiceless) %>%
  ggplot(aes(speaker, difference)) +
...
as_tibble(ranef(tra_lm)$speaker[2], rownames = "speaker") %>%
  ...
  geom_bar(stat = "identity") +
  labs(title = "By-speaker conditional means for phonation", y = "Tongue root position (mm)")
```

```{r tra-rand-lm-ranef-intercept}
as_tibble(ranef(tra_lm)$speaker[1], rownames = "speaker") %>%
  mutate(language = c(rep("Italian", 11), rep("Polish", 6))) %>%
  ggplot(aes(speaker, `(Intercept)`, fill = language)) +
  geom_bar(stat = "identity") +
  labs(title = "By-speaker conditional modes for tongue root position (intercept)", y = "Tongue root position (mm)")
```
...
as_tibble(coef(tra_lm)$speaker[2], rownames = "speaker") %>%
  ...
  ggplot(aes(speaker, c2_phonationvoiced, fill = language)) +
  geom_bar(stat = "identity") +
  labs(title = "By-speaker random coefficients for phonation", y = "Tongue root position (mm)")
```

```{r tra-rand-lm-coef-intercept}
as_tibble(coef(tra_lm)$speaker[1], rownames = "speaker") %>%
  mutate(language = c(rep("Italian", 11), rep("Polish", 6))) %>%
  ggplot(aes(speaker, `(Intercept)`, fill = language)) +
  geom_bar(stat = "identity") +
  labs(title = "By-speaker random intercepts", y = "Tongue root position (mm)")
```
...
```{r}
tra_series %>%
  ggplot(aes(proportion, tra, colour = vowel_duration, group = rec_date)) +
  geom_path(alpha = 0.5) +
...
```{r}
tra_series %>%
  ggplot(aes(proportion, tra, colour = c2_phonation)) +
  geom_smooth()
```

The general trend is that TR advances by 0.5 mm in voiceless and 1.5 mm in voiced but they start off at the same position.
...
```{r}
tra_series %>%
  ggplot(aes(proportion, tra, colour = c2_phonation, linetype = c2_phonation)) +
  geom_smooth(method = "auto") +
  facet_wrap(~ speaker, ncol = 4, scales = "free")
```

There is a lot of beween-speaker variation.
...
There is definitely an interaction with vowel and C2 place.
...
tra_gam <- bam(
  tra ~
    place_vowel_phon +
    s(proportion) +
    s(proportion, by = place_vowel_phon) +
...
```{r}
plot_smooths(tra_gam, proportion, voicing, place + vowel, split = list(place_vowel_phon = c("place", "vowel", "voicing")))
```

```{r}
acf_resid(tra_gam, split_pred = list(tra_series$rec_date))
```
...
```{r}
plot_smooths(tra_gam_2, proportion, voicing, vowel, split = list(vowel_phon = c("vowel", "voicing")))
```

```{r}
acf_resid(tra_gam_2, split_pred = list(tra_series$rec_date))
```
...
```{r}
plot_smooths(tra_gam_3, proportion, voicing, vowel, split = list(vowel_phon = c("vowel", "voicing")))
```

```{r}
plot_smooth(tra_gam_3, "proportion", plot_all = "vowel_phon", cond = list(vowel_duration = 35), rm.ranef = T)
```

```{r}
plot_smooth(tra_gam_3, "proportion", plot_all = "vowel_phon", cond = list(vowel_duration = 100), rm.ranef = T)
```

```{r}
plot_smooth(tra_gam_3, "proportion", plot_all = "vowel_phon", cond = list(vowel_duration = 200), rm.ranef = T)
```

```{r}
fvisgam(tra_gam_3, c("proportion", "vowel_duration"), rm.ranef = T, cond = list(vowel_phon = "a.voiceless"))
fvisgam(tra_gam_3, c("proportion", "vowel_duration"), rm.ranef = T, cond = list(vowel_phon = "a.voiced"))
```
...
tra_gam_4 <- bam(
  tra ~
    ...
    s(proportion, by = vowel) +
    s(vowel_duration) +
    s(vowel_duration, by = vowel) +
    ti(proportion, vowel_duration) +
    ti(proportion, vowel_duration, by = vowel) +
    s(proportion, speaker, bs = "fs", m = 1),
  data = tra_series
)
summary(tra_gam_4)
...
```{r}
fvisgam(tra_gam_3, c("proportion", "vowel_duration"), rm.ranef = T, cond = list(vowel = "a"))
```
...
Vowels followed by voiced consonants have a bigger difference.
C2 place and vowel have an effect.
There is no significant interaction of voicing and vowel duration.
Sentence duration is not significant.
