...
ita_pol <- token_measures %>%
  select(language, speaker, word, c2_phonation, v1_duration, c2_clos_duration, speech_rate_c) %>%
  rename(voicing = "c2_phonation")

eng <- eng_durations %>%
  filter(num_syl == "di") %>%
  mutate(language = "English") %>%
  select(language, speaker, word, voicing, v1_duration, c2_clos_duration, speech_rate_c) %>%
