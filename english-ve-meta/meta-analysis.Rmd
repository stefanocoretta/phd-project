...
meta_bm_1_est <- fixef(meta_bm)["Intercept","Estimate"]
meta_bm_1_q2.5 <- fixef(meta_bm)["Intercept","Q2.5"]
meta_bm_1_q97.5 <- fixef(meta_bm)["Intercept","Q97.5"]
...
bind_rows(estimated_tbl_final, studies_shrunk_1) %>%
  mutate(source = factor(source, levels = c("meta-analysis", "original"))) %>%
  ggplot(aes(reorder(study_ref, Estimate), Estimate, colour = source)) +
  geom_hline(aes(yintercept = 0), colour = "grey") +
...
```{r original-v-plot}
estimated_v_tbl %>%
  ggplot(aes(Estimate, reorder(study_ref, Estimate), colour = syl_pos)) +
  geom_point() +
  geom_errorbarh(aes(xmin = Q2.5, xmax = Q97.5), height = 0.2) +
  scale_color_brewer(type = "qual", palette = "Dark2")
```

## Voicing effect by syllable position within the word
