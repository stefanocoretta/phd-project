...
Using measurement error models with studies that reported standard deviations.
The model includes `se()`, where the standard error is calculated as $sd/\sqrt{N}$ (where $N$ is the number of observations from which the standard deviation was obtained).
`se(sigma = TRUE)` is used as suggested in the documentation (`?"addition-terms"`).
