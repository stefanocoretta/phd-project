...
estimated_tbl %>%
  ...
  annotate("rect", ymin = -Inf, ymax = Inf, xmin = meta_bm_q2.5, xmax = meta_bm_q97.5, alpha = 0.5, fill = "#a6cee3") +
  geom_point(size = 3) +
  annotate("polygon", x = c(meta_bm_est - 1.96 * e.max, meta_bm_est + 1.96 * e.max, meta_bm_est + 1.96 * e.min, meta_bm_est - 1.96 * e.min), y = c(1/e.max, 1/e.max, 1/e.min, 1/e.min), alpha = 0.2, fill = "orange") +
  scale_x_continuous(breaks = seq(-25, 150, 25)) +
  labs(
    x = "Difference in vowel duration (ms)",
    shape = "Syllable position"
