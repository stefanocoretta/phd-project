...
degg <- list.files(path = "./italian-polish/data/datasets/egg",
...
min_activity <- degg %>%
  group_by(language, c2_phonation_o, date) %>%
  summarise(
...
degg %>%
    ggplot(aes(x = proportion, colour = c2_phonation_o)) +
...
degg %>%
    ggplot(aes(x = proportion, colour = c2_phonation_o, linetype = c2_phonation_o)) +
        facet_wrap(~ speaker, nrow = 2) +
...
degg %>%
    ggplot(aes(proportion, closed_quotient, colour = c2_phonation_o, linetype = c2_phonation_o)) +
        facet_wrap(~ speaker, nrow = 2) +
...
degg %>%
  filter(c2_phonation_o == "voiceless") %>%
...
```{r cq-centre-speaker}
degg %>%
    ggplot(aes(proportion, quotient_centre, colour = c2_phonation_o, linetype = c2_phonation_o)) +
    geom_smooth() +
    facet_wrap(~ speaker, nrow = 2)
```
...
degg %>%
    ggplot(aes(proportion, weighted_quotient_centre, colour = c2_phonation_o, linetype = c2_phonation_o)) +
    geom_smooth() +
    facet_wrap(~ speaker, nrow = 2)
```
...
degg %>%
    ggplot(aes(proportion, peaks_ratio, colour = c2_phonation_o, linetype = c2_phonation_o)) +
  geom_smooth() +
  facet_wrap(~ speaker, nrow = 2) +
  xlim(0.1, 0.9)
```
...
```{r min-activity}
min_activity %>%
  ggplot(aes(c2_phonation_o, proportion, colour = language)) +
  geom_boxplot()
```
...
min_activity %>%
  ggplot(aes(c2_phonation_o, proportion, colour = speaker)) +
  geom_boxplot() +
  facet_grid(. ~ language)
```
...
degg %>%
    ggplot(aes(proportion, peaks_ratio_norm, colour = c2_phonation_o, linetype = c2_phonation_o)) +
    geom_smooth() +
    facet_wrap(~ language) +
  xlim(0.1, 0.9)
```
...
degg %>%
  filter(proportion > 0.45, proportion < 0.55) %>%
  ggplot(aes(c2_phonation_o, peaks_ratio_norm, colour = language)) +
  geom_boxplot() +
...
degg %>%
  ...
  ggplot(aes(peaks_ratio_norm, colour = c2_phonation_o)) +
  geom_density() +
  facet_grid(~ language)
```
...
degg %>%
  filter(proportion > 0.5) %>%
  ggplot(aes(speaker, peaks_ratio_norm, colour = c2_phonation_o)) +
  geom_boxplot()
```
...
Maybe the normalisation is obscuring differences?

```{r min-activity-norm}
min_activity_norm %>%
  ggplot(aes(c2_phonation, proportion, colour = language)) +
  geom_boxplot()
```

```{r min-activity-norm-lm}
min_activity_norm_lm <- lmer(
  proportion ~
    c2_phonation *
    language +
    (1|word),
  data = min_activity_norm
)

summary(min_activity_norm_lm)
```
