...
token_data <- token_measures %>%
  filter(c1_phonation == "voiceless") %>%
  mutate(c2_phonation = factor(c2_phonation, levels = c("voiceless", "voiced"))) %>%
  mutate_if(is.character, as.factor)

c1_data <- token_measures %>%
  filter(speaker %in% c("it01", "it02"), c2_phonation == "voiceless") %>%
  mutate(c1_phonation = factor(c1_phonation, levels = c("voiceless", "voiced"))) %>%
  mutate_if(is.character, as.factor) %>%
  droplevels()
...
voicing <- voicing <- list.files(
  path = "italian-polish/data/datasets/egg/",
  pattern = "*-voicing.csv",
  full.names = TRUE
) %>%
...
Vowel Onset to Release.

```{r}
duration %>%
  ggplot(aes(c2_phonation, vor)) +
  geom_boxplot()
```
...
```{r c1-rvofft-box-phonation}
duration %>%
  ggplot(aes(c2_phonation, c1_rvofft)) +
  geom_boxplot() +
  facet_grid(~ language) +
  labs(
    title = "Release to closure duration"
  )
```
...
closure_cv <- duration %>%
  ...
  summarise(
    ...
    cqv = (quantile(closure_duration, na.rm = TRUE, probs = 0.75) - quantile(closure_duration, na.rm = TRUE, probs = 0.25)) / (quantile(closure_duration, na.rm = TRUE, probs = 0.75) + quantile(closure_duration, na.rm = TRUE, probs = 0.25))
  )
```
...
## Stability measure (coefficient of variation) of RR and VOR

```{r}
rr_cv <- duration %>%
  group_by(language, speaker, c2_place, c2_phonation) %>%
  summarise(
    sd = sd(rel_rel, na.rm = TRUE),
    mean = mean(rel_rel, na.rm = TRUE),
    cv = 100 * sd(rel_rel, na.rm = TRUE) / mean(rel_rel, na.rm = TRUE),
    cqv = (quantile(rel_rel, na.rm = TRUE, probs = 0.75) - quantile(rel_rel, na.rm = TRUE, probs = 0.25)) / (quantile(rel_rel, na.rm = TRUE, probs = 0.75) + quantile(rel_rel, na.rm = TRUE, probs = 0.25))
  )
```
...
vor_cv <- duration %>%
  group_by(language, speaker, c2_place, c2_phonation) %>%
  summarise(
    sd = sd(vor, na.rm = TRUE),
    mean = mean(vor, na.rm = TRUE),
    cv = 100 * sd(vor, na.rm = TRUE) / mean(vor, na.rm = TRUE)
  )
```
...
Release to Release coefficient of variation is lower than Vowel Onset to Release (i.e. it is more stable).
...
anova(vor_lm_null, vor_lm)
```

```{r rr-lm-plot}
plot(allEffects(vor_lm))
```
...
The estimate for Release to Closure is very similar to that of Closure duration (about 15.5 msec).

# Vowel to Vowel interval

```{r vv}
token_data %>%
  ggplot(aes(v_v)) + geom_density() + geom_rug()
```

```{r vv-lang}
token_data %>%
  ggplot(aes(v_v)) + geom_density() + geom_rug() + facet_grid(~ language)
```

```{r vv-lang, warning=FALSE}
token_data %>%
  ggplot(aes(v_v, fill = c2_phonation)) +
  geom_density(alpha = 0.7, size = 0) + geom_rug() + facet_grid(language ~ .) +
  scale_fill_discrete(name = "C2 voicing") +
  labs(
    title = "Density plot of the V-to-V interval",
    x = "V to V duration (V1 onset to V2 onset)"
  )
```

# RVoffT of voiceless/voiced C1

```{r}
c1_data %>%
  group_by(speaker, c1_phonation) %>%
  summarise(n = n(), na = sum(is.na(c1_rvoff)))
```

```{r rvofft-dens}
c1_data %>%
  ggplot(aes(c1_rvoff, fill = c1_phonation)) +
  geom_density(alpha = 0.5) +
  geom_rug()
```
...
```{r rvofft-dens}
c1_data %>%
  ggplot(aes(c1_phonation, c1_rvoff, fill = c1_phonation)) +
  geom_boxplot() +
