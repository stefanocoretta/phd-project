---
title: "Tongue root trajectories"
...
```{r trp-gam-3-plot}
plot_smooths(trp_gam_3, proportion, c2_phonation_o) + scale_y_continuous(breaks = seq(1, 30, 1)) + scale_color_discrete() + scale_fill_discrete()
```
...
```{r trp-gam-4-plot}
fvisgam(trp_gam_4, c("proportion", "vowel_duration"), cond = list(c2_phonation_o = "voiceless"), rm.ranef = TRUE, zlim = c(5.4, 9.5))
fvisgam(trp_gam_4, c("proportion", "vowel_duration"), cond = list(c2_phonation_o = "voiced"), rm.ranef = TRUE, zlim = c(5.4, 9.5))
```
...
trp_gam_5 <- gam(
  trp ~
    language_voi_o +
    s(proportion) +
    s(proportion, by = language_voi_o) +
    s(proportion, speaker, bs = "fs", m = 1),
  data = kine_s
)
```
...
```{r trp-gam-5-plot}
plot_smooths(trp_gam_5, proportion, voicing, facet_terms = language, split = list(language_voi_o = c("language", "voicing"))) + scale_y_continuous(breaks = seq(1, 30, 1))
```
...
```{r trp-gam-6-plot}
fvisgam(trp_gam_6, c("proportion", "syl_rate"), rm.ranef = TRUE)
```

```{r trp-gam-6-plot-2}
plot_smooth(trp_gam_6, "proportion", cond = list("syl_rate" = 4), col = "red", rm.ranef = TRUE)
plot_smooth(trp_gam_6, "proportion", cond = list("syl_rate" = 5), col = "blue", rm.ranef = TRUE, add = TRUE)
plot_smooth(trp_gam_6, "proportion", cond = list("syl_rate" = 6), col = "green", rm.ranef = TRUE, add = TRUE)
plot_smooth(trp_gam_6, "proportion", cond = list("syl_rate" = 6.5), col = "orange", rm.ranef = TRUE, add = TRUE)
```
