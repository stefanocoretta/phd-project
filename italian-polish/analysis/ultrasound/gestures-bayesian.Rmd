...
gons_max_bm <- brm(
  ...
  data = gest_gmax,
  cores = 4,
  file = "./italian-polish/analysis/cache/gons_max_bm"
)
```
...
rel_gons_bm <- brm(
  ...
  data = gestures,
  cores = 4,
  file = "./italian-polish/analysis/cache/rel_gons_bm"
)
```
...
# Closing gesture duration (GONS to NONS)
...
Negative values are not possible (by definition, the gestural onset should always be earlier than the nucleus onset). This just means that the data is bad.
...
gestures %>%
  filter(closing > 35) %>%
  ggplot(aes(closing, fill = c2_phonation)) +
