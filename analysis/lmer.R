...
words <- read.csv("./pilot/stimuli/stimuli.csv")
results <- merge(vowels, words, by.x = "word", by.y = "word")

model <- lmer(duration ~ c2phonation * vowel + c1phonation + c2place + (1|word), data = results)

model <- lmer(duration ~ c2phonation * vowel + c2place + (1|word), data = results)
